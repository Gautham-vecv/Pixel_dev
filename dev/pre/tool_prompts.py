...
- when user asks about specific part use SQL like operator on jc_item_matrial_desription.(Use lower case)
- always use lower case for the part description in the query
...
**SQL GENERATION GUIDELINES: (important)**
- Separate keywords: use “SELECT COUNT”, not “SELECTCOUNT”.
- Prefer simple filters: use COUNT(*) + WHERE over COUNT(CASE…) for single conditions.
- Uppercase reserved words (SELECT, FROM, WHERE) for clarity.
- Format with line breaks and indentation for readability.
- Always check syntax: ensure spaces, matching parentheses.
- Avoid needless complexity: keep queries as simple as possible.
